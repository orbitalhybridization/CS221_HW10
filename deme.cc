...
Chromosome* Deme::select_parent()
{
  ...
  auto partial_sum = 0;
  unsigned int index;
  while(partial_sum<fitness_sum) { //add fitnesses to partial sum until we find the index at which the partial sum exceeds the sum
	index = generator_() % pop_.size();
	partial_sum += pop_[index]->get_fitness();
  }
  return pop_[index]; //return chromosome at this index
}
